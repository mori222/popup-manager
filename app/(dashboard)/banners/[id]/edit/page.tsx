...
export default function EditBannerPage({ params }: { params: { id: string } }) {
  ...
  const { sites, getBanner, updateBanner, isLoading } = useAppContext()
  ...
  useEffect(() => {
    const fetchBanner = () => {
      ...
      const banner = getBanner(Number.parseInt(params.id))
      ...
      form.reset({
        name: banner.name,
        url: banner.url,
        siteId: banner.siteId.toString(),
        displayRule: banner.displayRule,
        position: banner.position,
        closeButton: banner.showCloseButton,
        mobileDisplay: banner.showOnMobile,
  ...
  }, [params, router, form, getBanner, isLoading])
  ...
  const onSubmit = async (data: BannerFormValues) => {
    ...
    // バナーを更新
    updateBanner(Number.parseInt(params.id), {
      name: data.name,
    ...
    return (
      <div className="container mx-auto p-4 md:p-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight">バナーを編集</h1>
        </div>
